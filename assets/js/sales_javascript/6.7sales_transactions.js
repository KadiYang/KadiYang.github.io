...
async function loadPage(page = 1) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Sales request failed with status ${res.status}`);
    }
...
function renderTable(rows) {
  ...
  if (!body) {
    console.error("Missing #salesBody element to render sales transactions.");
    return;
  }
...
function renderPagination(current) {
  ...
  if (!div) {
    console.error("Missing #pagination container to render sales pagination.");
    return;
  }
...
if (goBackBtn) {
  ...
  console.error("Missing #goBack button for sales transactions.");
}
